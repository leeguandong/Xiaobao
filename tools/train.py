...
def parse_args():
    ...
    parser.add_argument('--config',
                        default=r'E:\comprehensive_library\mmaction2_add\configs\recognition\timesformer\timesformer_divST_video_8x32x1_15e_kinetics400_rgb.py',
    ...
    parser.add_argument(
        '--validate',
        action='store_true',  # 在训练期间每 k 个周期进行一次验证（默认值为 5，可通过修改每个配置文件中的 evaluation 字典变量的 interval 值进行改变）
        help='whether to evaluate the checkpoint during training')
    parser.add_argument(
        '--test-last',
        action='store_true',  # 在训练结束后使用最后一个检查点的参数进行测试，将测试结果存储在 ${WORK_DIR}/last_pred.pkl 中。
        help='whether to test the checkpoint after training')
    parser.add_argument(
        '--test-best',
        action='store_true',  # 在训练结束后使用效果最好的检查点的参数进行测试，将测试结果存储在 ${WORK_DIR}/best_pred.pkl 中。
        help=('whether to test the best checkpoint (if applicable) after '
              'training'))
    group_gpus = parser.add_mutually_exclusive_group()  # 创建互斥组，保证互斥组中的参数只能出现一个
    group_gpus.add_argument(
        '--gpus',
        type=int,
        help='number of gpus to use '  # 使用的 GPU 数量，仅适用于非分布式训练。
    ...
    group_gpus.add_argument(
        ...
        type=int,
        nargs='+',
        help='ids of gpus to use '  # 使用的 GPU ID，仅适用于非分布式训练。
             '(only applicable to non-distributed training)')
    ...
    parser.add_argument(
        ...
        default='none',  # 分布式任务初始化启动器选项。可选值有 none，pytorch，slurm，mpi。特别地，如果被设置为 none, 则会以非分布式模式进行测试。
        help='job launcher')
