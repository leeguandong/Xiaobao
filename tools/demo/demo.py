...
def parse_args():
    ...
    parser.add_argument('--video', default=r'E:\comprehensive_library\mmaction2_add\data\test\qingjiang.mp4',
                        help='video file/url or rawframes directory')
    parser.add_argument('--label', default=os.path.join(path, r'\tools\data\kinetics\label_map_k400.txt'),
                        help='label file')
    parser.add_argument(
        '--cfg-options',
        nargs='+',
    ...
    parser.add_argument(
        ...
        default=False,
        action='store_true',
        help='whether to use rawframes as input')  # 如指定，代表用帧目录作为输入，否则代表使用视频作为输入
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='specify fps value of the output video when using rawframes to '
             'generate file')  # 使用帧目录作为输入时，代表输入的帧率。默认为 30。
    parser.add_argument(
        '--font-scale',
        default=0.5,
        type=float,
        help='font scale of the label in output video')  # 输出视频上的字体缩放比例。默认为 0.5。
    parser.add_argument(
        '--font-color',
        default='white',
        help='font color of the label in output video')  # 输出视频上的字体颜色，默认为白色（ white）。
    parser.add_argument(
        '--target-resolution',
        nargs=2,
        default=None,
        ...
        help='Target resolution (w, h) for resizing the frames when using a '
             ...
             'resized by keeping the existing aspect ratio')  # 输出视频的分辨率，如未指定，使用输入视频的分辨率。
    parser.add_argument(
        '--resize-algorithm',
        default='bicubic',
        help='resize algorithm applied to generate video')  # 缩放视频时使用的插值方法，默认为 bicubic。
    parser.add_argument('--out-filename',
                        default=r"E:\comprehensive_library\mmaction2_add\data\cache\qingjiang_tsn.mp4",
                        help='output filename')  # 输出视频的路径，如未指定，则不会生成输出视频。
