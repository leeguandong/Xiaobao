...
def parse_args():
    ...
    parser.add_argument(
        '--config',
        default=os.path.join(path, r"configs\detection\ava\slowonly_omnisource_pretrained_r101_8x8x1_20e_ava_rgb.py"),
        # 时空检测配置文件路径
        help='spatio temporal detection config file path')
    parser.add_argument(
        '--checkpoint',  # 时空检测模型权重文件路径
        default=os.path.join(path, r'weights\slowonly_omnisource_pretrained_r101_8x8x1_20e_ava_rgb_20201217-16378594.pth'),
        help='spatio temporal detection checkpoint file/url')
    parser.add_argument(
        '--det-config',  # 人体检测配置文件路径
        default=os.path.join(path, r'demo\faster_rcnn_r50_fpn_2x_coco.py'),
        help='human detection config file path (from mmdet)')
    parser.add_argument(
        '--det-checkpoint',  # 人体检测模型权重文件路径
        default=os.path.join(path, r'weights\faster_rcnn_r50_fpn_2x_coco_bbox_mAP-0.384_20200504_210434-a5d8aa15.pth'),
        help='human detection checkpoint file/url')
    parser.add_argument(
        '--det-score-thr',
        type=float,
        default=0.9,  # 人体检测分数阈值，默认为 0.9
        help='the threshold of human detection score')
    parser.add_argument(
        '--action-score-thr',
        type=float,
        default=0.5,  # 动作检测分数阈值，默认为 0.5
        help='the threshold of human action score')
    parser.add_argument(
        '--video',
        default=r'E:\comprehensive_library\mmaction2_add\data\test\panshouwan.mp4',
        help='video file/url')
    parser.add_argument(
        '--label-map',
        default=r'E:\comprehensive_library\mmaction2_add\tools\data\ava\label_map.txt',  # 标签映射文件
        help='label map file')
    parser.add_argument(
        '--device', type=str, default='cpu', help='CPU/CUDA device option')  # cpu/cuda:0
    parser.add_argument(
        '--out-filename',
        default=r'E:\comprehensive_library\mmaction2_add\data\cache\demo_spat_det.mp4',
        help='output filename')
    parser.add_argument(
        '--predict-stepsize',  # 每 N 帧进行一次预测（以节约计算资源），默认值为 8
        default=8,
        type=int,
