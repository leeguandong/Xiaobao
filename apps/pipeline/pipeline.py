...
from apps.pipeline.utils import print_arguments, merge_cfg, PipeTimer
from apps.pipeline.utils import get_test_images, crop_image_with_det, crop_image_with_mot, parse_mot_res, parse_mot_keypoint
...
from python.visualize import visualize_box_mask, visualize_attr, visualize_pose, visualize_action, visualize_speed, visualize_team, \
    visualize_singleplayer, visualize_boating, visualize_ball, visualize_link_player, visualize_golf, visualize_player_rec, \
    visualize_ball_control
...
class PipePredictor(object):
    ...
    def __init__(self, args, cfg, is_video=True, multi_camera=False):
        ...
        if not is_video:
            ...
            if self.with_skeleton_action: # skeleton之前必须配有关键点
    ...
    def predict_video(self, video_file):
        ...
        while (1):
            ...
            # idbased和skeletonbased都是要依赖追踪模型tracking
            if self.modebase["idbased"] or self.modebase["skeletonbased"]:
                ...
                if self.with_skeleton_action: # 基于谷歌点的单帧动作识别
                    if frame_id > self.warmup_frame:
