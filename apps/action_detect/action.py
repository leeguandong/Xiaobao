...
sys.path.append("/home/ivms/local_disk/mmaction2_add/apps/action_detect")
...
class ActionDetection(object):
    ...
    def infer(self, imgs_path, pcm_path, fps=5):
        ...
        # step2: get proposal
        bmn_results = self.extract_proposal(video_features) # 433段 {'start': 2494.0, 'end': 2601.0, 'score': 0.516885457083651}
    ...
    def extract_feature(self):
        ...
        if not self.DEBUG:
            ...
            image_features = self.image_model.predict(self.configs) # (174832,2048)
            if self.PCM_ONLY: # 只提取模拟信号
                sample_rate = self.configs['AUDIO']['sample_rate'] # 16000
                pcm_features = mfcc_extractor.extract_pcm(self.pcm_path, sample_rate) # （5827,50,64）
                audio_features = []
            else:
